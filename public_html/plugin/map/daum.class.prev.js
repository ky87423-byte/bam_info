...
var nf_map = function() {
	...
	//basic_location : header_meta.php에서 실행합니다.
...
// : 지도불러오기
	this.map_start = function(id, arr) {
		if(!arr) arr = basic_location;
		if(!arr['lat']) arr['lat'] = basic_location['lat'];
		if(!arr['lng']) arr['lng'] = basic_location['lng'];
...
// : 지도등록시 사용
	...
	this.customoverlay_delete = function() {
		for(x in nf_map.customoverlay) {
			nf_map.customoverlay[x].setMap(null);
		}
	}
...
// : 좌표로 주소값 가져오기
	this.get_latlng_address = function(lat, lng, json) {
		if(!nf_map.geocoder) nf_map.geocoder = new kakao.maps.services.Geocoder();
		nf_map.geocoder.coord2Address(lng, lat, function(result, status) {
			if (status === kakao.maps.services.Status.OK) {
				// : 도로명 우선
	...
	this.address_move_marker = function(lat, lng, json) {
		...
		if(!lat) lat = basic_location['lat'];
		if(!lng) lng = basic_location['lng'];
...
// : 드래그가 끝날 때 발생한다.
			...
			if(data.detail_view=='Y') {
				var markers = $(data.positions).map(function(i, position) {
					...
					kakao.maps.event.addListener(marker, 'click', function(){
						if(!nf_map.customoverlay[position.no]) {
							nf_map.customoverlay[position.no] = new kakao.maps.CustomOverlay({
								content: position.map_content,
								map: nf_map.map,
								position: marker.getPosition()
							});
						}
						nf_map.customoverlay[position.no].setMap(nf_map.map);
					});
					return marker;
				});
				// 클러스터러에 마커들을 추가합니다
				...
				var markers = $(data.positions).map(function(i, position) {
					...
					nf_map.customoverlay[position.no] = new daum.maps.CustomOverlay({
						position: latlng,
						map:nf_map.map,
						clickable: true,
						xAnchor: 0.5,
	...
	this.close_overlay = function(no) {
		nf_map.customoverlay[no].setMap(null);
	}
