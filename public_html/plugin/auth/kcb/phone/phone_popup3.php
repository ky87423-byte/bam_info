...
<?php
	...
	// ########################################################################
	$CP_CD = $env['wr_hphone_id'];				// 회원사코드(아이디)
    ...
    //' 라이센스 파일
    //'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
	$license = NFE_PATH.'/plugin/auth/kcb/'.$CP_CD."_IDS_01_".$target."_AES_license.dat";
	...
	**************************************************************************/
	...
	if($ret == 0) {		// 함수 실행 성공일 경우 변수를 결과에서 얻음
		...
		if( $RSLT_CD == "B000" ) { // B000 : 정상건
			...
			if($env['wr_use_adult'] && $RSLT_BIRTHDAY>date("Ymd", strtotime("-20 year"))) {
				die('<script type="text/javascript">alert("미성년자는 이용할 수 없습니다.");window.close();</script>');
			} else {
				$_SESSION['_return_auth_'] = $output;
				$move_url = $_SESSION['__auth_click_page__'];
				if(!$move_url) $move_url = NFE_URL.'/member/register.php';
				
				die('<script type="text/javascript">opener.location.href="'.$move_url.'";window.close();</script>');

				// : 성공
				if($member['mb_id']) { // 회원이라면
					if($RSLT_BIRTHDAY<=date("Ymd", strtotime("-20 year"))) { // 성인맞으면 성인저장
						$db->_query("update snow_member set is_adult=1 where `no`=?", array($member['no']));
					}
				}
			}
		}
	...
	die('<script type="text/javascript">alert("실명인증이 실패되었습니다.\n운영자에게 문의하시기 바랍니다.");window.close();</script>');
	exit;
?>
