...
function selectedFiles(evt) {
	var M_int = allowedMaxImgSize/1024/1024;
	if(allowedMaxImgSize<evt.target.files[0].size) {
		alert(M_int+'M이하로 첨부해주세요.');
		return false;
	}
...
DoUpload.prototype = {
    ...
    imageResize : function (image, filetype, resizeWidth, orientation, addWaterMark) {
        var canvas, source_w = image.width, source_h = image.height,
            ...
            ctx = null, angle, gl, imageData = null;
        ...
        if (orientation === 6 || orientation === 8 || orientation === 3) {
            ...
            if (orientation !== 3) {
                canvas.width = source_h; canvas.height = source_w;
            } else {
            ...
            ctx.rotate(angle);
            ctx.drawImage(image, -source_w / 2, -source_h / 2);
    ...
    dataLoadHandler: function (evt) {
        ...
        if (evt.target.readyState === FileReader.DONE) {
            ...
            image.onload = function () {
				...
				var orgimgSize = this.width;

				if (orgimgSize > imageResizeWidth) {
...
function init(dialog) {
	...
	var is_news = "";	
	var news_url = parent.document.URL;
	if( news_url.indexOf('insert.php') > -1 || news_url.indexOf('regist.html') > -1){
		is_news = "?is_news=1";
	}
    ...
    uploadScript = oEditor.config.editorPath + 'imageUpload/upload.php' + is_news;
