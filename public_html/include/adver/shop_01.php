...
$n_cnt = 3;
if ($env['service_config_arr']['shop'][$service_k]['width'] > $n_cnt)
	$n_cnt = $env['service_config_arr']['shop'][$service_k]['width'];
if (strpos($_SERVER['PHP_SELF'], '/include/') !== false)
	$n_cnt = $env['service_config_arr']['shop'][$service_k]['sub_width'];
...
<section class="pri ">
	<h2><img src="../images/icon/sub_h2_img01.png"
			alt="아이콘"><?php echo $nf_shop->service_name['shop']['main'][0]; ?><b>많은 고객님께서 인정한 최고의 업체입니다.</b></h2>
	<!--n3 ~ n6 한줄 가로 li 갯수 / 관리자에서 지정-->
	<ul class="list_item pri_m n<?php echo $n_cnt; ?>">
		<?php
		switch ($total['c'] <= 0) {
			case true:
				?>
				<li class="no_info">프리미엄 업체 내역이 없습니다.</li>
				<?php
				break;

			default:
				$cnt = 0;
				$shop_li_arr = array(); // 🔥 li 저장용
				...
				while ($shop_row = $db->afetch($shop_query)) {
					...
					?>
					<li class="common"
						onClick="location.href='<?php echo NFE_URL; ?>/shop/index.php?no=<?php echo $shop_row['no']; ?>'">
						<div>
							<div class="item_img">
								<p class="img">
								...
								<?php if ($shop_row['wr_best']) { ?>
									<p class="best_icon">
										<img src="../images/best_icon.png" alt="best아이콘">
									</p>
								<?php } ?>
								...
								<?php if ($env['use_shop_tema'] && $shop_info['tema_cnt'] > 0) { ?>
									<div class="shadow">
										<ul>
											<?php
											if (is_array($shop_info['tema_arr'])) {
												foreach ($shop_info['tema_arr'] as $v) {
													?>
								...
								<p class="location location-latlng">
							...
							<div class="item_info">
								<ul class="tag">
									...
									<?php if ($shop_row['wr_rdate'] >= $shop_new_time) { ?>
										<li class="sticker">NEW</li>
									<?php } ?>
									<?php if ($shop_info['coupon_use_check']) { ?>
										<li class="sticker c_p">쿠폰</li>
									<?php } ?>
								</ul>

								<button type="button" class="heart_tap scrap-star-" code="shop" no="<?php echo $shop_row['no']; ?>">
									<i
										class="axi <?php echo $nf_util->is_scrap($shop_row['no'], 'shop') ? 'axi-heart2' : 'axi-heart'; ?>"></i>
								</button>
								...
								<ul class="ev">
									...
									<?php if ($env['use_shop_guide']) { ?>
										<li class="commu">
											<i class="axi axi-ion-chatbubble-working"></i>
											<a href="<?php echo NFE_URL; ?>/include/review.php?pno=<?php echo $shop_row['no']; ?>">
												<?php echo $shop_row['wr_guide_int']; ?><em>/ 후기</em>
											</a>
										</li>
									<?php } ?>
								</ul>
								...
								<?php if ($env['use_shop_price']) { ?>
									<ul class="product">
										<?php if ($shop_row['wr_sale'] > 0) { ?>
											<li class="sale"><span><?php echo $shop_row['wr_sale']; ?></span>%</li>
										<?php } ?>
										<li class="price"><span><?php echo number_format($shop_row['wr_price']); ?></span>원</li>
										<?php if ($shop_row['wr_sale'] > 0) { ?>
											<li class="d_price"><?php echo number_format($shop_row['wr_price1']); ?>원</li>
										<?php } ?>
									</ul>
								<?php } ?>
							</div>
