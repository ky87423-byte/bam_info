...
$nf_util->sess_page_save("individual_list");

$_GET['type'] = 'individual';
$where_arr = $nf_search->member();
$_where = $where_arr['where'];

$q = "nf_member as nm where mb_left=0 and mb_left_request=0 and is_delete=0 ".$_where;
...
$_SESSION['admin_mem_individual_q'] = $q.$order;
$mem_query = $db->_query("select * from ".$_SESSION['admin_mem_individual_q']." limit ".$paging['start'].", ".$_arr['num']);
?>
<style type="text/css">
...
<div class="wrap">
	...
	<section>
		...
		<div class="consadmin conbox">
			...
			<h6>개인회원 관리<span>총 <b><?php echo number_format(intval($_arr['total']));?></b>명의 회원이 검색되었습니다.</span>
				<p class="h6_right">
			...
			<div class="table_top_btn">
				...
				<button type="button" class="gray" onClick="select_email_box(this, 'email-')">메일전송</button>
				<a href="../regist.php?mode=excel_admin_mem_individual_q"><button type="button" class="gray"><img src="../../images/ic/xls.gif" alt=""> 엑셀저장</button></a>
			...
			<table class="table4">
				...
				<thead>
					<tr>
						...
						<th><a href="#none" onClick="click_sort('mb_name', '<?php echo $mb_name_order;?>')">이름 (성별/나이)<?php echo $mb_name_order=='desc' ? '▲' : '▼';?></a></th>
					...
					<?php
						...
						default:
							while($mem_row=$db->afetch($mem_query)) {
								$update_url = './individual_insert.php';
			...
			<div class="table_top_btn bbn">
				...
				<a href="../regist.php?mode=excel_admin_mem_individual_q"><button type="button" class="gray"><img src="../../images/ic/xls.gif" alt=""> 엑셀저장</button></a>
				<button type="button" class="blue"><strong>+</strong> 회원등록</button></a>
			</div>
			</form>
		</div>
		<!--//conbox-->
