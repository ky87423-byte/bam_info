...
var open_guide_box = function(el, no) {
	...
	obj.find(".guide-comment-").css({"display":"none"});
	$(form).find(".btn-write-").html("수정하기");
	if(get_code=='comment') {
		obj.find(".guide-comment-").css({"display":"table-row"});
		$(form).find(".btn-write-").html("등록하기");
	}

	var title_ = get_code ? '답변' : '수정';
	$(".popup_box-.guide-").find("h6").find("em").html(title_);

	$.post("../regist.php", "&mode=open_guide_box&no="+no+"&code="+get_code, function(data){
		data = $.parseJSON(data);
...
<input type="hidden" name="mode" value="guide_answer_write" />
<input type="hidden" name="no" value="" />
<input type="hidden" name="code" value="" />
<div class="layer_pop conbox popup_box- guide-" style="display:none;">
	<div class="h6wrap">
		<h6>이용후기 <em>답변</em></h6>
		<button type="button" class="close" onClick="open_guide_box(this, 'none')">X 창닫기</button>
	</div>
	<table>
		<colgroup>
			<col width="17%">
			...
			<tr>
				...
				<td>
					<table class="answer_intable bt" >
						...
						<tr>
							<th>공감</th>
							<td><input type="text" name="good" value="" class="long100"></td>	
							<th>별점</th>
							<td>
								<select name="point">
								<?php
	...
	<div class="pop_btn">
		<button type="submit" class="blue btn-write-">등록하기</button>
		<button type="button" class="gray" onClick="open_guide_box(this, 'none')">창닫기</button>
	</div>
</div>
</form>