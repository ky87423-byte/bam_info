...
$q = "nf_scrap as ns right join nf_employ as ne on ns.`pno`=ne.`no` where ns.`code`='employ' ".$_where;
	...
	<section>
		...
		<div class="consadmin conbox">
			<div class="guide">
				...
				<ul>
					<li>- 개인회원들의 채용공고 스크랩 현황을 관리하시는 페이지 입니다.</li>
				</ul>
			</div>
			...
			<h6>채용공고 스크랩 관리<span>총 <b><?php echo number_format(intval($_arr['total']));?></b>건이 검색되었습니다.</span>
				<p class="h6_right">
			...
			<table class="table4">
				...
				<thead>
					<tr>
						...
						<th>회원구분</th>
						<th>회원정보</th>
						<th colspan="2">스크랩한 채용(정규직)공고</th>
						<th>스크랩일</th>
						<th>편집</th>
					</tr>
				</thead>
				<tbody>
					...
					<tr class="tac">
						...
						<td><a href="<?php echo NFE_URL;?>/employ/employ_detail.php?no=<?php echo $row['pno'];?>" class="blue" target="_blank"><?php echo $nf_util->get_text($row['wr_subject']);?></a></td>
						<td>
							<ul>
						...
						<td style="text-align:center">
							<button type="button" class="gray common" onclick="nf_util.ajax_post(this, '삭제하시겠습니까?')" no="<?php echo $row['ns_no'];?>" mode="delete_scrap" url="<?php echo NFE_URL;?>/include/regist.php" para="code=employ"><i class="axi axi-minus2"></i> 삭제</button>
						</td>
					</tr>
					<?php
							}
