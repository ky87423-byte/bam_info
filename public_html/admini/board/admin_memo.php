...
$q = "nf_message as nm where (pmb_id='_admin_' or mb_id='_admin_') ".$_where;
...
var click_use = function(el) {
	$.post(root+"/admini/regist.php", "mode=use_message&val="+el.value, function(data){
		data = $.parseJSON(data);
...
var reply_message = function(el, no) {
	...
	form.admin.value = 1;
	$.post(root+"/include/regist.php", "mode=get_message_info&code=received&no="+no, function(data){
		data = $.parseJSON(data);
		if(data.msg) alert(data.msg);
		if(data.move) location.href = data.move;
		try{
	...
	<section>
		...
		<div class="consadmin conbox">
			<div class="guide">
				...
				<ul>
					<li>- 회원이 관리자에게 쪽지를 발송한 리스트입니다.</li>
					<li>- 우측에 쪽지발송 버튼을 통해 회원에게 답 쪽지를 보낼수 있습니다. .</li>
						...
						default:
							while($row=$db->afetch($message_query)) {
								...
								if($db->is_table($_table) && $row['code']!='reply') {
									...
									$con1 = $nf_shop->kind_of[$row['code']];
								}
					?>
					<tr>
						<td><input type="checkbox" name="chk[]" class="chk_" value="<?php echo $row['no'];?>"></td>
						<td>
							<?php if($row['mb_id']=='_admin_') {?>
							관리자
							<?php } else {?>
							<a href="#none" onClick="member_mno_click(this)" class="blue fwb" mno="<?php echo $row['mno'];?>"><?php echo $row['mb_id'];?></a>
							<?php }?>
							=>
							<?php if($row['pmb_id']=='_admin_') {?>
							관리자
							<?php } else {?>
							<a href="#none" onClick="member_mno_click(this)" class="blue fwb" mno="<?php echo $row['pmno'];?>"><?php echo $row['pmb_id'];?></a>
							<?php }?>
						</td>
						...
						<td>
							<?php if($row['mno']) {?>
							<button type="button" class="gray common" onclick="reply_message(this, '<?php echo $row['no'];?>')"><i class="axi axi-minus2"></i> 쪽지발송</button>
							<?php }?>
							<button type="button" class="gray common" onclick="nf_util.ajax_post(this, '삭제하시겠습니까?')" no="<?php echo $row['no'];?>" mode="delete_message" url="../regist.php"><i class="axi axi-minus2"></i> 삭제</button>
						</td>
					</tr>
					<?php
							}
